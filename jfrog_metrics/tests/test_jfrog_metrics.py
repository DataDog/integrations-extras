...
def test_openmetrics_art_check(aggregator, mock_art_agent_data):
    ...
    aggregator.assert_metric("jfrog_artifactory.runtime_heap_processors", count=1, value=6)
    aggregator.assert_metric("jfrog_artifactory.db_connections_active", count=1, value=2)
...
def test_openmetrics_xray_check(aggregator, mock_xray_agent_data):
    ...
    aggregator.assert_metric("jfrog_xray.sys_memory_used", count=1, value=20)
