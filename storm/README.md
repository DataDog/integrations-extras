...
If you are using Agent v6.8+ follow the instructions below to install the Storm check on your host. See our dedicated Agent guide about [how to install Community integration][2] to see how to install them with the [Agent prior v6.8][3] or the [Docker Agent][4]:
...
1. Edit the `storm.d/conf.yaml` file in the `conf.d/` folder at the root of your [Agent's configuration directory][9] to start collecting your Storm [metrics](#metrics).
  See the [sample storm.d/conf.yaml][10] for all available configuration options.

2. [Restart the Agent][11]

## Validation

[Run the Agent's `status` subcommand][12] and look for `storm` under the Checks section.
...
[10]: https://github.com/DataDog/integrations-extras/blob/master/storm/datadog_checks/storm/data/conf.yaml.example
...
[13]: https://github.com/DataDog/integrations-extras/blob/master/storm/metadata.csv
