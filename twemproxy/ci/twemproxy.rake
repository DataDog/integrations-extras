...
namespace :ci do
  namespace :twemproxy do |flavor|
    ...
    task install: ['ci:common:install'] do
      ...
      sleep_for 5
      # set etcd config
      sh %(curl -L -X PUT http://127.0.0.1:2379/v2/keys/services/redis/01 -d value="#{docker_addr}:6101")
      sh %(curl -L -X PUT http://127.0.0.1:2379/v2/keys/services/redis/02 -d value="#{docker_addr}:6102")
      sh %(curl -L -X PUT http://127.0.0.1:2379/v2/keys/services/twemproxy/port -d value="6100")
      sh %(curl -L -X PUT http://127.0.0.1:2379/v2/keys/services/twemproxy/host -d value="#{docker_addr}")
      # publish the redis host:ip information into etcd
      sh %(docker run --name=twemproxy -d -p 6100:6100 -p 6222:6222 -e ETCD_HOST=#{docker_addr}:4001 jgoodall/twemproxy)
