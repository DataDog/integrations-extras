...
def test(aggregator, dd_run_check, instance):
    ...
    if NEO4J_VERSION.startswith('3.5'):
        ...
        aggregator.assert_metric('neo4j.page_cache.usage_ratio', tags=['db_name:neo4j', f'endpoint:{METRICS_URL}'])
        aggregator.assert_metric('neo4j.page_cache.usage_ratio', tags=['db_name:system', f'endpoint:{METRICS_URL}'])
    else:
