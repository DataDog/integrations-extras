...
def test_connect_exception(dd_run_check, aggregator, caplog):
    with pytest.raises(Exception, match="Failed to resolve 'invalid-hostname'|Max retries exceeded"):
        check = ScaleopsCheck('scaleops', {}, [BAD_HOSTNAME_INSTANCE])
        dd_run_check(check)

    aggregator.assert_service_check('scaleops.openmetrics.health', ServiceCheck.CRITICAL)


def test_check_mock_scaleops_metrics(dd_run_check, aggregator, check, mock_prometheus_metrics):
    dd_run_check(check)
