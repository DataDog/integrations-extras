...
class TestLinuxVMExtras(AgentCheckTest):
    ...
    def test_check(self):
        ...
        with mock.patch('__builtin__.open',
                        return_value=open(Fixtures.file('proc_vmstat_1', sdk_dir=self.FIXTURE_DIR), 'r'), autospec=True):
            self.run_check(config)

        # Run the check twice to compute `monotonic_count`s
        time.sleep(1)
        with mock.patch('__builtin__.open',
                        return_value=open(Fixtures.file('proc_vmstat_2', sdk_dir=self.FIXTURE_DIR), 'r'), autospec=True):
            self.run_check(config)
