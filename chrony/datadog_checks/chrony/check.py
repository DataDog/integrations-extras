...
class ChronyCheck(AgentCheck):
    ...
    def _find_chronyc(self):
        ...
        try:
            result = subprocess.run(['which', 'chronyc'], 
                                  capture_output=True, 
                                  text=True, 
                                  check=True)
    ...
    def _get_chrony_tracking(self):
        ...
        try:
            result = subprocess.run([self.chronyc_path, 'tracking'], 
                                  capture_output=True, 
                                  text=True, 
                                  check=True,
                                  timeout=30)
    ...
    def _parse_chrony_output(self, output):
        ...
        for fieldname, regex_pattern, description, unit in self.fields:
    ...
    def check(self, instance):
        ...
        try:
            ...
            for fieldname, value in metrics.items():
                ...
                unit = None
                for field_name, regex_pattern, description, field_unit in self.fields:
                ...
                self.log.debug(f"Submitted metric {metric_name}: {value} {unit or ''}")
            
            # Submit a service check for chrony availability
            self.service_check('chrony.can_connect', AgentCheck.OK, tags=tags)
            
            self.log.info(f"Successfully collected {len(metrics)} chrony metrics")
            
        except Exception as e:
            self.log.error(f"Error collecting chrony metrics: {e}")
            self.service_check('chrony.can_connect', AgentCheck.CRITICAL, 
                             message=str(e)) 