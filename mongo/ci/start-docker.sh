...
echo 'docker exec -it $NAME mongo --eval "printjson(rs.config()); printjson(rs.status());" localhost:$PORT'
docker exec -it $NAME mongo --eval "printjson(rs.config()); printjson(rs.status());" localhost:$PORT
