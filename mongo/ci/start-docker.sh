...
SHARD03_ID=$(docker run --link=$NAME -p $PORT3:$PORT3 --name $NAME3 -d mongo mongod --bind_ip 0.0.0.0 --port $PORT3)
SHARD03_IP=$(docker inspect ${SHARD03_ID} | grep '"IPAddress"' | cut -d':' -f2 | cut -d'"' -f2)
SHARD03_IP=$(echo $SHARD03_IP | cut -d " " -f2)
...
echo "Your shard container ${SHARD03_ID} listen on ip: ${SHARD03_IP}:$PORT3 (waiting that becomes ready)"
until docker logs ${SHARD03_ID} | grep "waiting for connections" >/dev/null;
do
    sleep 2
done

echo 'docker exec -it $NAME mongo --eval "printjson(rs.initiate()); printjson(rs.conf());" localhost:$PORT'
docker exec -it $NAME mongo --eval "printjson(rs.initiate()); printjson(rs.conf());" localhost:$PORT

echo 'docker exec -it $NAME mongo --eval "printjson(rs.add(\'$SHARD01_IP:$PORT1\\')); printjson(rs.add(\'$SHARD02_IP:$PORT2\\')); printjson(rs.status());" localhost:$PORT'
docker exec -it $NAME mongo --eval "printjson(rs.add('$SHARD01_IP:$PORT1')); printjson(rs.add('$SHARD02_IP:$PORT2')); printjson(rs.status());" localhost:$PORT
