...
@pytest.mark.parametrize("ip, is_public", [
...
def test_records_skips_cached_ips(stream):
    ...
    with patch("requests.post") as mock_post, \
        ...
        stream.check.log.info.assert_any_call(
            "SCAMALYTICS: SKIP %s (persistent cache <%sh)", "1.1.1.1", 24
        )
