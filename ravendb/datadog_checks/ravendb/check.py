...
class RavendbCheck(OpenMetricsBaseCheckV2):
    ...
    def __init__(self, name, init_config, instances):
        ...
        if not enable_database_metrics:
            desired["skipDatabasesMetrics"] = "true"
        if not enable_index_metrics:
            desired["skipIndexesMetrics"] = "true"
        if not enable_collection_metrics:
            desired["skipCollectionsMetrics"] = "true"
        if include_gc_metrics:
            desired["includeGcMetrics"] = "true"

        if desired:
            parsed = urlparse(endpoint)
            query = dict(parse_qsl(parsed.query, keep_blank_values=True))
            query.update(desired)
            self.instance["openmetrics_endpoint"] = urlunparse(parsed._replace(query=urlencode(query)))
