...
from .common_no_db_no_idx_no_collec_no_gc import METRICS_NO_DB_NO_IDX_NO_COLLEC_NO_GC as BASELINE_METRICS

pytestmark = [pytest.mark.usefixtures("dd_environment"), pytest.mark.e2e]
REQUIRED_ENV = ["RAVEN_License"]

# Database, index, collection and GC metrics are opt-in, so the default e2e instance never emits them.
OPTIONAL_METRICS = EXPECTED_PROMETHEUS_METRICS - BASELINE_METRICS
...
def test_e2e_ravendb_check_e2e_assert_metrics(dd_run_check, aggregator, check):
    ...
    # Baseline metrics are always emitted and must be present.
    for metric_name in BASELINE_METRICS:
        aggregator.assert_metric(metric_name, at_least=1)

    # Tiered metrics are opt-in; only assert they are known, not that they appear.
