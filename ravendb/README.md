...
This integration collects metrics from RavenDB through its Prometheus/OpenMetrics endpoint, giving you visibility into cluster health, node activity, database operations, indexing performance, and resource utilization. These metrics can be used for monitoring, visualization, and alerting within Datadog.
...
RavenDB exposes metrics in [Prometheus/OpenMetrics][6] format at an administrative endpoint: `/admin/monitoring/v1/prometheus`. To collect metrics:

1. Edit the `ravendb/conf.yaml` file in the `conf.d/` directory of the Datadog Agent. For all available configuration options, see the [sample ravendb/conf.yaml][7].
...
2. (Optional) Enable additional metric groups:
