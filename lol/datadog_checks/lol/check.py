...
class LolCheck(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        super(LolCheck, self).__init__(name, init_config, instances)

        # Use self.instance to read the check configuration
        # self.url = self.instance.get("url")
        self.url = "http://127.0.0.1:2999/liveclientdata/allgamedata"
    ...
    def check(self, _):
        ...
        for statName, statValue in response_json['activePlayer']['championStats'].items():
            self.gauge(statName, statValue, tags=['summoner_name:' + response_json['activePlayer']['summonerName']])
