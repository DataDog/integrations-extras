...
- Converts Shopify's `clicked` DOM event into a RUM click action.
- Converts Shopify's `ui_extension_errored` event (a checkout UI extension crashing) into a RUM error, tagged with the failing extension's name, target, type and the app the extension belongs to.
